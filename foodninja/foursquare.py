...
class FoursquareAuth(object):
    ...
    def get_auth_redirect(self):
        ...
        return ('{0}?client_id={1}&response_type=code'
                '&redirect_uri={2}').format(AUTH_ENDPOINT, self.client_id,
                                            self.redirect_uri)

    def get_auth_token(self, code):
        """Get access token with code from foursquare"""
        url = ('{0}?client_id={1}&client_secret={2}'
               '&grant_type=authorization_code&code={3}').format(TOKEN_ENDPOINT,
                                                                 self.client_id,
                                                                 self.client_secret,
                                                                 code)
...
class Foursquare(object):
    ...
    def request(self, resource, resource_id=None, aspect=None, action=None,
                **kwargs):
        try:
